  ...
  it.effect("reports the legacy migration stage", () =>
    withStore(
      Effect.gen(function* () {
        ...
        assert.equal(
          error.catalogPath,
          environment.path.join(environment.stateDir, "connection-catalog.json"),
        );
        assert.instanceOf(
          error.cause,
        ...
        assert.equal(
          error.message,
          `Legacy desktop saved-environment migration failed during read-legacy-registry into ${environment.path.join(environment.stateDir, "connection-catalog.json")}.`,
        );
