...
describe("DesktopAppIdentity", () => {
  it.effect("uses Janus user data without probing a legacy T3 Code path", () =>
    withIdentity(
      Effect.gen(function* () {
        ...
        assert.equal(
          userDataPath,
          environment.path.join("/Users/alice", "Library", "Application Support", "janus"),
        );
      }),
  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Janus"]);
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Janus");
