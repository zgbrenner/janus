...
describe("versionSkew", () => {
  ...
  it("returns a mismatch when the server version differs from the client", () => {
    expect(resolveVersionMismatch("9.9.9")).toEqual({
      ...
      serverVersion: "9.9.9",
      hint: "Version mismatch. Try syncing the client and server to the same Janus version.",
    });
  });
  ...
  it("appends a hint to connection errors when versions differ", () => {
    ...
    expect(appendVersionMismatchHint("Socket closed.", mismatch)).toBe(
      "Socket closed. Hint: Version mismatch. Try syncing the client and server to the same Janus version.",
    );
  });
  ...
  it("matches version-drift guidance to the advertised update path", () => {
    ...
    expect(serverUpdateGuidance("desktop-managed", "Desktop server")).toBe(
      "The Desktop server is run by the Janus desktop app on its machine — update the desktop app there to sync them.",
    );
    expect(serverUpdateGuidance(null, "Local server")).toBe(
      "Install the Janus release from the copied link on that machine to sync them.",
    );
  });

  it("hands manual updaters the Janus release download link, never an npm command", () => {
    expect(manualServerUpdateUrl("1.2.3")).toBe(
      "https://github.com/zgbrenner/janus/releases/tag/v1.2.3",
    );
    expect(manualServerUpdateUrl("  ")).toBe("https://github.com/zgbrenner/janus/releases");
  });
});
